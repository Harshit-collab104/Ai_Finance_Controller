...
def compute_financial_ratios(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["total_equity"] = np.maximum(5.0, df["total_assets"] - df["total_debt"] - df["accounts_payable"])
    
    # Estimated interest expense and EBITDA
    df["interest_expense"] = np.maximum(0.5, (df["short_term_debt"] * 0.08 + df["long_term_debt"] * 0.06) / 4.0)
    ...
    df["quick_ratio"] = np.where(df["current_liabilities"] > 0, (df["cash"] + df["accounts_receivable"]) / df["current_liabilities"], np.nan)
    df["cash_ratio"] = np.where(df["current_liabilities"] > 0, df["cash"] / df["current_liabilities"], np.nan)

    # 2. Profitability Ratios
    df["gross_margin"] = np.where(df["revenue"] > 0, df["gross_profit"] / df["revenue"], np.nan)
    df["operating_margin"] = np.where(df["revenue"] > 0, df["operating_income"] / df["revenue"], np.nan)
    df["net_profit_margin"] = np.where(df["revenue"] > 0, df["net_income"] / df["revenue"], np.nan)
    df["roa"] = np.where(df["total_assets"] > 0, df["net_income"] / df["total_assets"], np.nan)
    df["roe"] = np.where(df["total_equity"] > 0, df["net_income"] / df["total_equity"], np.nan)

    # Growth Metrics (grouped by company)
    df["revenue_growth"] = df.groupby("company_id")["revenue"].pct_change().fillna(0.0)
    df["net_income_growth"] = df.groupby("company_id")["net_income"].pct_change().fillna(0.0)

    # 3. Leverage Ratios
    df["debt_to_equity"] = np.where(df["total_equity"] > 0, df["total_debt"] / df["total_equity"], np.nan)
    df["debt_to_ebitda"] = np.where(df["ebitda"] > 0, df["total_debt"] / df["ebitda"], np.nan)
    df["interest_coverage"] = np.where(df["interest_expense"] > 0, df["operating_income"] / df["interest_expense"], np.nan)

    # 4. Cash Flow Ratios & Indicators
    df["free_cash_flow"] = df["operating_cash_flow"] - df["capital_expenditure"]
    df["cash_burn_rate"] = np.where(df["operating_cash_flow"] < 0, np.abs(df["operating_cash_flow"]), 0.0)
    ...
    df["ocf_trend_slope"] = df.groupby("company_id")["operating_cash_flow"].transform(
        lambda s: s.rolling(3).apply(get_slope, raw=False)
    ).fillna(0.0)

    # Round ratio columns for clean display
    ratio_cols = [
        "current_ratio", "quick_ratio", "cash_ratio", "gross_margin",
        "operating_margin", "net_profit_margin", "roa", "roe", "revenue_growth",
        "net_income_growth", "debt_to_equity", "debt_to_ebitda", "interest_coverage",
        "free_cash_flow", "cash_burn_rate", "cash_flow_growth", "ocf_trend_slope"
    ]
    for c in ratio_cols:
        df[c] = df[c].round(4)
